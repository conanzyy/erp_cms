...
<title><?php echo isset($this->site_config['web_site_name'])?$this->site_config['web_site_name']:'';?>------咨询修改</title>
...
<body>
   ...
   <input type="hidden" name="id" value="<?php echo $info['id'];?>">
    ...
    <ul class="forminfo">
    <li><label style="width:142px">名称<b>*</b></label><input type='text' class="dfinput" name='title' value="<?php echo $info['title'];?>"></li>
       <li><label style="width:142px">类型<b>*</b></label>
        <div class="usercity">
  		...
  		?>
  		<option value="<?php echo $c_v['id'] ;?>" <?php if($info['typeid'] == $c_v['id']){echo "selected";}?>><?php echo $c_v['html'];?><?php echo $c_v['typename']  ; ?></option>
  		<?php 
  			}
  		}	
  		?>
  	    ...
  	    <li><label style="width:142px">关键词<b>*</b></label><input type='text' class="dfinput" name='keysword' value="<?php echo $info['keysword'];?>"><i>请用,号分割</i></li>
     	<li><label style="width:142px">权重<b>*</b></label><input type='text' class="dfinput" name='weight' value="<?php echo $info['weight'];?>"><i>请填写数字</i></li>
     	<li><label style="width:142px">点击数量<b>*</b></label><input type='text' class="dfinput" name='click' value="<?php echo $info['click'];?>"><i>请填写数字</i></li>
     	<li><label style="width:142px">属性<b>*</b></label>
     	<?php 
     	$flagData = explode("," , $info['flag']);
     		...
     		<?php 
				if(isset($this->news_attr)){
					foreach($this->news_attr as $attr_key => $attr_val){
						$checked = '';
						$checked = (in_array($attr_key , $flagData))?"checked":'';
						echo "<input type='checkbox' name='flag[]' value='{$attr_key}' {$checked}>".$attr_val."&nbsp;&nbsp;" ;
					}
				}
			?>
     	</li>
  
		<li><label style="width:142px ;">缩略图：</label>
		<input type='file' readonly class="dfinput" name='image' >
		<?php if($info['image']){?>
		<img src="<?php echo __UPLOAD__DO_MAIN;?>/news/<?php echo $info['image'];?>" width="180px" height="180px">
		<?php }?>	
		</li>
	   
	   
	    
    	<li><label style="width:142px">添加日期</label><input type='text' class="dfinput" name='create_date' onFocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="<?php echo date("Y-m-d H:i:s" , $info['create_date']);?>"></li>
        <li><label style="width:142px ;">内容：</label>
        <textarea class="dfinput" id="content" name="content"><?php echo $info['content'];?></textarea>
        
        
			
		</li>
		<li><label style="width:142px">介绍<b>*</b></label>
		<textarea class="dfinput"  name="introduce"><?php echo $info['introduce'];?></textarea>
		</li>
