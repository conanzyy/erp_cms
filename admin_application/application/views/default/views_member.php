...
<title><?php echo isset($this->site_config['web_site_name'])?$this->site_config['web_site_name']:'';?>-------网站用户管理</title>
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li class="click" onclick="add()"><span><img src="<?php echo  _IMGPATH_ ;?>t01.png" /></span>添加网站用户</li>
        </ul>
  ...
  <ul class="seachform" style="margin-left:4%">
    ...
    <table class="tablelist">
        ...
        <th>编号<i class="sort" field="uid" orderby = "desc"  style="cursor:pointer" ><img src="<?php echo  _IMGPATH_ ;?>/px.gif" /></i></th>
        <th>用户名</th>
        <th >注册日期   <i class="sort" field="regtime" orderby = "desc"  style="cursor:pointer" ><img src="<?php echo  _IMGPATH_ ;?>/px.gif" /></i></th>
        <th>状态   <i class="sort" field="status" orderby = "desc"  style="cursor:pointer" ><img src="<?php echo  _IMGPATH_ ;?>/px.gif" /></i></th>
        <th>有效期<i class="sort" field="expire" orderby = "desc"  style="cursor:pointer" ><img src="<?php echo  _IMGPATH_ ;?>/px.gif" /></i></th>
        <th>操作</th>
        </tr>
        </thead>
        <tbody id="result_">
...
var loadding_0  ;
var page = 1 ;
var field = "uid" ; //  哪个字段排序
var orderby = "desc" ; //升序还是降序
$(function () {
	common_request();
});
function common_request(){
	var url="<?php echo site_url("member/index");?>?inajax=1";
	var data_ = {
		'page':page,
		'time':<?php echo time();?>,
		'action':'ajax_data' , 
	...
	success: function(msg){
		...
		if(msg.resultcode<0){
			...
			for(var i in list){
				shtml+='<tr>';
				shtml+= '<td><input name="" type="checkbox" value="'+list[i].uid+'" /></td>' ;
				...
				shtml+='<td><a href="<?php echo site_url("member/edit") ;?>?id='+list[i].uid+'" class="tablelink">编辑信息</a>&nbsp;&nbsp;</td>';
				shtml+='</tr>';
			}
			$("#result_").html(shtml);
...
function add(){
	$.layer({
	    ...
	    offset: ['20px',''],
	    area: ['720px','460px'],
	    iframe: {src: "<?php echo site_url("member/add");?>?showpage=1"}
	}); 
}
